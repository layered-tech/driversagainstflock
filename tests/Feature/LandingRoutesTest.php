...
test('map page uses the V3 full screen map surface', function () {
    ...
    expect($mapPage)
        ...
        ->toContain("map.value.on('sourcedata', handleMarkerSourceData)")
        ->toContain("event.sourceId !== 'source-markers'")
        ->toContain('!event.isSourceLoaded')
        ->toContain('queueMarkerConeSync')
        ->toContain('markerConeSyncFrameId')
