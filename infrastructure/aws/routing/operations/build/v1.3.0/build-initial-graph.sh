...
readonly OPERATION_VERSION="1.3.0"
...
publish_telemetry() {
    ...
    local cpu_used_percent="0.00"
    local current_cpu_total
    local current_cpu_idle
    ...
    read -r current_cpu_total current_cpu_idle < <(
        awk '/^cpu / {
            total = 0
    ...
    if [[ -f "${CPU_SAMPLE_FILE}" ]]; then
        read -r previous_cpu_total previous_cpu_idle < "${CPU_SAMPLE_FILE}" || true

        if [[ "${previous_cpu_total:-}" =~ ^[0-9]+$ && "${previous_cpu_idle:-}" =~ ^[0-9]+$ ]] \
            && (( current_cpu_total > previous_cpu_total && current_cpu_idle >= previous_cpu_idle )); then
            cpu_used_percent="$(awk \
                -v current_total="${current_cpu_total}" \
                -v current_idle="${current_cpu_idle}" \
                -v previous_total="${previous_cpu_total}" \
                -v previous_idle="${previous_cpu_idle}" \
                'BEGIN {
                    total_delta = current_total - previous_total
                    idle_delta = current_idle - previous_idle
                    used = 100 * (total_delta - idle_delta) / total_delta
                    if (used < 0) used = 0
    ...
    printf '%s %s\n' "${current_cpu_total}" "${current_cpu_idle}" > "${CPU_SAMPLE_FILE}.tmp"
    chmod 0644 "${CPU_SAMPLE_FILE}.tmp"
    mv -f "${CPU_SAMPLE_FILE}.tmp" "${CPU_SAMPLE_FILE}"
    ...
    jq -n \
        ...
        --argjson cpu_used_percent "${cpu_used_percent}" \
    ...
    jq -n \
        ...
        '[
            ...
            {MetricName: "BuilderCpuUsed", Dimensions: [{Name: "ReleaseId", Value: $release_id}], Unit: "Percent", Value: $cpu_used_percent},
...
telemetry_loop() {
    ...
    while true; do
        ...
        publish_telemetry || true
        loop_elapsed_seconds="$(( $(date +%s) - loop_started_epoch ))"
        sleep_seconds="$(( TELEMETRY_INTERVAL_SECONDS - loop_elapsed_seconds ))"
