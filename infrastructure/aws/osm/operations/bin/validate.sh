...
for removed_current_source in \
    "${OSM_DOWNLOAD_PATH}/north-america-bootstrap.osm.pbf" \
    "${OSM_DOWNLOAD_PATH}/north-america-bootstrap.osm.pbf.md5"; do
...
history_cleanup_marker="${OSM_STATE_PATH}/history-bootstrap-source-removed.complete"
if [[ -f "${history_cleanup_marker}" ]]; then
    require_file "${history_cleanup_marker}"
    for removed_history_source in \
        "${OSM_DOWNLOAD_PATH}/history-bootstrap.osm.pbf" \
        "${OSM_DOWNLOAD_PATH}/history-bootstrap.osm.pbf.md5"; do
...
current_sequence="$(state_sequence "${OSM_STATE_PATH}/current-replication.state")"
history_sequence="$(state_sequence "${OSM_STATE_PATH}/history-replication.state")"
...
marker_path="${OSM_STATE_PATH}/validation.complete"
