...
[[ "${backup_changeset_sequence:-}" =~ ^[0-9]+$ ]] \
    || die 'Verified backup pre-dump changeset sequence is invalid'
...
write_pipeline_state last_successful_backup_changeset_sequence "${backup_changeset_sequence}"
...
log "Recorded durable verified-backup marker at node ${backup_shared_sequence}/${backup_current_sequence}/${backup_history_sequence} and changeset ${backup_changeset_sequence} sequences"
