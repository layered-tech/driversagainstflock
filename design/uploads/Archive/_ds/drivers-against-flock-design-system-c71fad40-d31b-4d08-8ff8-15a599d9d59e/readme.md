...
```jsx
const { Icon } = window.DesignSystem_c71fad;
...
| `templates/daf/` | Ready-to-copy full product screens: `WebLanding`, `WebFullMap`, `MobileApp` (marked as templates) plus supporting screens (blog, docs, legal, map building blocks). Each loads the system via `ds-base.js`. |
| `SKILL.md` | Agent-Skill manifest for using this system downstream. |

**Components** are React (`window.DesignSystem_c71fad.<Name>`), styled entirely through the CSS custom properties above. Each has a `.d.ts` (props) and `.prompt.md` (usage).
