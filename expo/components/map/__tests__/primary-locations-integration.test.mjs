...
describe('Home and Work map search integration', () => {
    ...
    test('adds configured Home and Work locations to idle car headers', () => {
        ...
        assert.match(
            autoPlaySource,
            /function startAutoPlayNavigation[\s\S]*?const rollbackNavigationStart[\s\S]*?activeNavigationRoute = null;[\s\S]*?activeNavigationDestination = null;[\s\S]*?updateRootTemplateHeaderActions\(\);/,
        );
        assert.match(
            autoPlaySource,
